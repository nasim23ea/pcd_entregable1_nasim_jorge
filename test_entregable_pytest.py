...
class MiembroDepartamento(): 
    ...
    def _eliminar_miembro(self, persona):
        ...
        try:
            for p in self.miembros[persona.departamento.value]:
                if p.dni == persona.dni:
                    self.miembros[persona.departamento.value].remove(p)
...
class Universidad:
    ...
    def contratar_profesor(self, profesor):
        try:
            ...
            print(f"Error inesperado al contratar al profesor: {e}")
    
    def contratar_investigador(self, investigador):
        try:
            for p in self.profesores["asociados"] + self.profesores["titulares"]:
            ...
            for lista in self.profesores.values():
                for p in lista:
                    if p.dni == dni:
                        ...
                        for asignatura in copia: #eliminar como docente de las asignaturas. Usamos copia por que si no no podemos iterar bien al actualizar la lista de valores.
    ...
    def matricular_asignaturas(self, estudiante, *asignaturas):
            ...
            try:
                ...
                for asignatura in asignaturas: 
                    ...
                    if asignatura in estudiante.asignaturas_matriculadas: 
                        raise ValueError(f'{estudiante.nombre} ya está matriculado en {asignatura.nombre}')
                    else:
                        estudiante._matricular_asignatura(asignatura)
                        asignaturas_matriculadas.append(asignatura.nombre)
                if asignaturas_matriculadas:
                    print(f"{estudiante.nombre} se ha matriculado de las siguientes asignaturas: {', '.join(asignaturas_matriculadas)}.")
        ...
        try:
            ...
            for asignatura in asignaturas:
                ...
                if asignatura in estudiante.asignaturas_matriculadas: 
                    ...
                    raise ValueError(f'{estudiante.nombre} no está matriculado en {asignatura.nombre}')
            if asignaturas_desmatriculadas:
                print(f"{estudiante.nombre} se ha desmatriculado de las siguientes asignaturas: {', '.join(asignaturas_desmatriculadas)}.")
        except Exception as e: # Mantener la captura genérica si no se anticipan errores específicos
            print(f"Error inesperado: {e}")


    def agregar_miembro_departamento(self, persona):# #agregar una persona a un departamento no tendria sentido
        self.miembros_departamento._añadir_miembro(persona)         #ya que los miembros (profesores, investigadores...) se añaden automaticamente
                                                      #al crearlos. Lo que si tiene sentido es su modificacion (enunciado)
    
    def eliminar_miembro_departamento(self, persona):#
        self.miembros_departamento._eliminar_miembro(persona)
    ...
    def get_miembros_departamento(self, dep = None):
            try:
                if dep is not None and not isinstance(dep, Departamento):
                    ValueError("El departamento debe pertenecer a la clase Departamento")
                elif dep is not None:
                    print(f"\n\n------Departamento : {dep.name}------")
                    for e in self.miembros_departamento.miembros[dep.value]:
                        print(e.devuelve_datos())
    ...
    def definir_asignatura(self, asignatura):
            try:
                if not isinstance(asignatura, Asignatura):
                    raise TypeError('La entrada debe corresponder con la clase Asignatura')
                for a in self.asignaturas:
                    if a.id == asignatura.id: # los id's se asignan de manera automatica
                        raise ValueError(f'La asignatura {a.nombre} asociada al identificador {a.id} ya estaba registrada previamente')
                self.asignaturas.append(asignatura)
                print(f'{asignatura.nombre} con id {asignatura.id} ha sido añadida correctamente como asignatura')
            except Exception as e:
                print(f"Ocurrio un error inesperado:{e}")
...
# Area investigacion
def test_definir_area_investigacion():#
    areas = [
    ...
    universidad = Universidad('UMU', 1,[MLI,PCD],[Departamento.DIIC, Departamento.DIS, Departamento.DITEC],areas)
    universidad.definir_areas_investigacion('Aprendizaje Reforzado')
    assert 'Aprendizaje Reforzado' in universidad.areas_investigacion

def test_eliminar_area_investigacion():
    areas = [
    "Aprendizaje automático y análisis predictivo",
    "Visualización de datos y análisis de redes complejas"
    ]
    ...
    PCD = Asignatura(2,'Programacion Para Ciencia De Datos', 6, Departamento.DIIC)
    universidad = Universidad('UMU', 1,[MLI,PCD],[Departamento.DIIC, Departamento.DIS, Departamento.DITEC],areas)
    universidad.eliminar_area_investigacion("Visualización de datos y análisis de redes complejas")
    assert "Visualización de datos y análisis de redes complejas" not in universidad.areas_investigacion

# Asignatura
def test_definir_asignaturas():#
    areas = [
    "Aprendizaje automático y análisis predictivo",
    "Visualización de datos y análisis de redes complejas"
    ]
    ...
    universidad = Universidad('UMU', 1,[MLI,PCD],[Departamento.DIIC, Departamento.DIS, Departamento.DITEC],areas)
    universidad.definir_asignatura(SYS)
    assert SYS in universidad.asignaturas
...
def test_eliminar_asignatura():
    ...
    PCD = Asignatura(2,'Programacion Para Ciencia De Datos', 6, Departamento.DIIC)
    universidad = Universidad('UMU', 1,[MLI,PCD],[Departamento.DIIC, Departamento.DIS, Departamento.DITEC],areas)
    universidad.eliminar_asignatura(PCD.id)
    assert PCD not in universidad.asignaturas
...
def test_matricular_estudiante():
    ...
    PCD = Asignatura(2,'Programacion Para Ciencia De Datos', 6, Departamento.DIIC)
    universidad = Universidad('UMU', 1,[MLI,PCD],[Departamento.DIIC, Departamento.DIS, Departamento.DITEC],areas)
    estudiante1 = Estudiante('Jorge Ballesta Cerezo', '49336768E', 'C/Aljufera, N12', Sexo.HOMBRE, [PCD, MLI])
    universidad.matricular_estudiante(estudiante1)
    assert estudiante1 in universidad.estudiantes

def test_matricular_asignaturas_estudiante(): # hay que retocar la funcion
    areas = [
    "Aprendizaje automático y análisis predictivo",
    "Visualización de datos y análisis de redes complejas"
    ]
    ...
    PCD = Asignatura(2,'Programacion Para Ciencia De Datos', 6, Departamento.DIIC)
    universidad = Universidad('UMU', 1,[MLI,PCD],[Departamento.DIIC, Departamento.DIS, Departamento.DITEC],areas)
    estudiante1 = Estudiante('Jorge Ballesta Cerezo', '49336768E', 'C/Aljufera, N12', Sexo.HOMBRE, [])
    universidad.matricular_estudiante(estudiante1)
    universidad.matricular_asignaturas(estudiante1, MLI, PCD)
    assert MLI in estudiante1.asignaturas_matriculadas and PCD in estudiante1.asignaturas_matriculadas

def test_desmatricular_asignaturas_estudiante(): 
    areas = [
    "Aprendizaje automático y análisis predictivo",
    "Visualización de datos y análisis de redes complejas"
    ]
    ...
    PCD = Asignatura(2,'Programacion Para Ciencia De Datos', 6, Departamento.DIIC)
    universidad = Universidad('UMU', 1,[MLI,PCD],[Departamento.DIIC, Departamento.DIS, Departamento.DITEC],areas)
    estudiante1 = Estudiante('Jorge Ballesta Cerezo', '49336768E', 'C/Aljufera, N12', Sexo.HOMBRE, [MLI, PCD])
    universidad.matricular_estudiante(estudiante1)
    universidad.desmatricular_asignaturas(estudiante1, MLI, PCD)
    assert MLI not in estudiante1.asignaturas_matriculadas and PCD not in estudiante1.asignaturas_matriculadas
...
def test_eliminar_estudiante():
    ...
    assert estudiante1 not in universidad.estudiantes

# Profesor
def test_contratar_profesores():
    areas = [
    "Aprendizaje automático y análisis predictivo",
    "Visualización de datos y análisis de redes complejas"
    ]
    ...
    PCD = Asignatura(2,'Programacion Para Ciencia De Datos', 6, Departamento.DIIC)
    universidad = Universidad('UMU', 1,[MLI,PCD],[Departamento.DIIC, Departamento.DIS, Departamento.DITEC],areas)
    profesor_asociado1 = ProfesorAsociado('Benito Ubeda', '49445325M', 'C/Angeles, N10',Sexo.HOMBRE, Departamento.DIS, [MLI])
    profesor_titular1 = ProfesorTitular('Jorge Bernal Bernabe', '54365725H', 'C/Sajon, N31',Sexo.HOMBRE, Departamento.DIIC, [PCD], areas[0]) 
    universidad.contratar_profesor(profesor_asociado1)
    universidad.contratar_profesor(profesor_titular1)
    assert profesor_asociado1 in universidad.profesores['asociados'] and profesor_asociado1 not in universidad.profesores['titulares']
    assert profesor_asociado1 in MLI.docentes
    assert profesor_titular1 in universidad.profesores['titulares'] and profesor_titular1 not in universidad.profesores['asociados']
    assert profesor_titular1 in PCD.docentes

def test_despedir_profesores():
    areas = [
    "Aprendizaje automático y análisis predictivo",
    "Visualización de datos y análisis de redes complejas"
    ]
    ...
    PCD = Asignatura(2,'Programacion Para Ciencia De Datos', 6, Departamento.DIIC)
    universidad = Universidad('UMU', 1,[MLI,PCD],[Departamento.DIIC, Departamento.DIS, Departamento.DITEC],areas)
    profesor_asociado1 = ProfesorAsociado('Benito Ubeda', '49445325M', 'C/Angeles, N10',Sexo.HOMBRE, Departamento.DIS, [PCD])
    universidad.contratar_profesor(profesor_asociado1)
    universidad.despedir_profesor('49445325M')
    assert profesor_asociado1 not in universidad.profesores['asociados'] 
    assert profesor_asociado1 not in PCD.docentes

def test_asignar_profesor_asignatura():
    areas = [
    "Aprendizaje automático y análisis predictivo",
    "Visualización de datos y análisis de redes complejas"
    ]
    ...
    profesor_asociado1 = ProfesorAsociado('Benito Ubeda', '49445325M', 'C/Angeles, N10',Sexo.HOMBRE, Departamento.DIS, [PCD])
    universidad.contratar_profesor(profesor_asociado1)
    universidad.asignar_profesor_asignatura(profesor_asociado1, MLI,PCD)
    assert profesor_asociado1 in MLI.docentes and profesor_asociado1 in PCD.docentes

def test_eliminar_profesor_asignatura():
    areas = [
    "Aprendizaje automático y análisis predictivo",
    "Visualización de datos y análisis de redes complejas"
    ]
    ...
    PCD = Asignatura(2,'Programacion Para Ciencia De Datos', 6, Departamento.DIIC)
    universidad = Universidad('UMU', 1,[MLI,PCD],[Departamento.DIIC, Departamento.DIS, Departamento.DITEC],areas)
    profesor_asociado1 = ProfesorAsociado('Benito Ubeda', '49445325M', 'C/Angeles, N10',Sexo.HOMBRE, Departamento.DIS, [PCD, MLI])
    universidad.contratar_profesor(profesor_asociado1)
    universidad.eliminar_profesor_asignatura(profesor_asociado1, PCD, MLI)
    assert profesor_asociado1 not in PCD.docentes and profesor_asociado1 not in MLI.docentes

# Investigador
def test_contratar_investigador():
    areas = [
    "Aprendizaje automático y análisis predictivo",
    "Visualización de datos y análisis de redes complejas"
    ]
    ...
    PCD = Asignatura(2,'Programacion Para Ciencia De Datos', 6, Departamento.DIIC)
    universidad = Universidad('UMU', 1,[MLI,PCD],[Departamento.DIIC, Departamento.DIS, Departamento.DITEC],areas)
    investigador1 = Investigador('Jose Perez Ruiz', '23543276P', 'C/Despanto, N12', Sexo.HOMBRE, Departamento.DIS, areas[0])
    universidad.contratar_investigador(investigador1)
    assert investigador1 in universidad.investigadores
def test_despedir_investigador():
    areas = [
    "Aprendizaje automático y análisis predictivo",
    "Visualización de datos y análisis de redes complejas"
    ]
    ...
    investigador1 = Investigador('Jose Perez Ruiz', '23543276P', 'C/Despanto, N12', Sexo.HOMBRE, Departamento.DIS, areas[0])
    universidad.contratar_investigador(investigador1)
    universidad.despedir_investigador('23543276P')
    assert investigador1 not in universidad.investigadores

# Miembro de Departamento
def test_agregar_miembro_dpto():
    areas = [
    "Aprendizaje automático y análisis predictivo",
    "Visualización de datos y análisis de redes complejas"
    ]
    ...
    investigador1 = Investigador('Jose Perez Ruiz', '23543276P', 'C/Despanto, N12', Sexo.HOMBRE, Departamento.DIS, areas[0])
    profesor_titular1 = ProfesorTitular('Jorge Bernal Bernabe', '54365725H', 'C/Sajon, N31',Sexo.HOMBRE, Departamento.DIIC, [PCD], areas[0])    
    universidad.agregar_miembro_departamento(investigador1)
    universidad.agregar_miembro_departamento(profesor_asociado1)
    universidad.agregar_miembro_departamento(profesor_titular1)
    assert profesor_titular1 in universidad.miembros_departamento.miembros[Departamento.DIIC.value]
    assert profesor_asociado1 in universidad.miembros_departamento.miembros[Departamento.DIS.value]
    assert investigador1 in universidad.miembros_departamento.miembros[Departamento.DIS.value]

def test_cambiar_miembro_dpto():
    areas = [
    "Aprendizaje automático y análisis predictivo",
    "Visualización de datos y análisis de redes complejas"
    ]
    ...
    profesor_titular1 = ProfesorTitular('Jorge Bernal Bernabe', '54365725H', 'C/Sajon, N31',Sexo.HOMBRE, Departamento.DIIC, [PCD], areas[0])
    universidad.contratar_profesor(profesor_titular1) 
    universidad.cambio_departamento(profesor_titular1, Departamento.DIS)
    assert profesor_titular1 in universidad.miembros_departamento.miembros[Departamento.DIS.value]
    assert profesor_titular1 not in universidad.miembros_departamento.miembros[Departamento.DIIC.value]
   


def test_eliminar_miembro_dpto():
    areas = [
    "Aprendizaje automático y análisis predictivo",
    "Visualización de datos y análisis de redes complejas"
    ]
    ...
    profesor_titular1 = ProfesorTitular('Jorge Bernal Bernabe', '54365725H', 'C/Sajon, N31',Sexo.HOMBRE, Departamento.DIIC, [PCD], areas[0])
    universidad.contratar_profesor(profesor_titular1)
    universidad.eliminar_miembro_departamento(profesor_titular1)
    assert profesor_titular1 not in universidad.miembros_departamento.miembros[Departamento.DIIC.value]
...
##comentario: Las areas de investigacion pueden ser más de una, por lo que SIEMPRE tienen que ser una lista (aunque haya solo un área de investigacion)
    ...
    universidad.definir_asignatura(Asignatura(6, "Optimizacion II", 6, Departamento.DIS))
    print([asignatura.nombre for asignatura in universidad.asignaturas])

    universidad.definir_asignatura(Asignatura(2, "Optimizacion II", 6, Departamento.DIS))
...
#############################################################################################
    ...
    universidad.matricular_asignaturas(estudiante1, BDII)

    # agregar matricular de asignaturas a alumno

    del universidad

    test_cambiar_miembro_dpto()